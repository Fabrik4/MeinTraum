...
function formatMonthYear(dateString: string) {
  return new Date(dateString).toLocaleDateString("de-CH", { month: "long", year: "numeric" })
}

function formatDate(dateString: string) {
  return new Date(dateString).toLocaleDateString("de-CH", { day: "numeric", month: "short" })
}
...
function getEntryDate(entry: TimelineEntry): string {
  if (entry.type === "dream") return entry.dreamed_at || entry.created_at
  ...
  async function fetchAll() {
    ...
    if (!dreamsRes.error && dreamsRes.data) {
      setDreams(dreamsRes.data.map((d: any) => ({
        ...
        type: "dream",
        persons: [...new Set(
          d.dream_entry_entities
            ?.filter((e: any) => e.user_entities?.entity_type === "person")
            .map((e: any) => e.user_entities.entity_label) ?? []
        )],
        places: [...new Set(
          d.dream_entry_entities
            ?.filter((e: any) => e.user_entities?.entity_type === "place")
            .map((e: any) => e.user_entities.entity_label) ?? []
        )],
      })))
    }
    ...
    <main className="min-h-screen bg-[#070b14] px-6 py-16 text-white">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-12 flex flex-wrap items-start justify-between gap-6">
          <div>
            ...
            <p className="mt-3 text-sm leading-7 text-white/50">
              Träume und Stimmungen im Verlauf – dein inneres Archiv.
            </p>
          </div>
        ...
        {!loading && allEntries.length === 0 && (
          <div className="rounded-3xl border border-white/10 bg-white/5 p-8 text-center text-white/50 backdrop-blur">
        ...
        {!loading && Object.entries(grouped).map(([month, entries]) => (
          <section key={month} className="mb-14">
            ...
            <div className="relative space-y-5">
              ...
              {entries.map((entry) => {
                if (entry.type === "dream") return <DreamCard key={`d-${entry.id}`} dream={entry} />
                return <JournalCard key={`j-${entry.id}`} journal={entry} />
              })}
            </div>
          </section>
        ))}
...
function DreamCard({ dream }: { dream: DreamEntry }) {
  ...
  return (
    <div className="relative flex gap-5">
      {/* Timeline-Punkt cyan für Träume */}
      ...
      <div className="flex-1 rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur transition hover:border-white/20">
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {dream.dream_clarity && (
              <span className="text-xs text-white/30">✨ {dream.dream_clarity}</span>
            )}
          </div>
        ...
        {hasTags && (
          <div className="flex flex-wrap gap-2">
            {tone === "nightmare" && (
              <span className="rounded-full border border-red-300/20 bg-red-300/10 px-3 py-1 text-xs text-red-100">Albtraum</span>
            )}
            {tone === "pleasant" && (
              <span className="rounded-full border border-emerald-300/20 bg-emerald-300/10 px-3 py-1 text-xs text-emerald-100">Schöner Traum</span>
            )}
            {emotions.map((em) => (
              <span key={em} className="rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1 text-xs text-cyan-100">💭 {em}</span>
            ))}
            {dream.persons.map((p) => (
              <span key={p} className="rounded-full border border-violet-300/20 bg-violet-300/10 px-3 py-1 text-xs text-violet-100">👤 {p}</span>
            ))}
            {dream.places.map((p) => (
              <span key={p} className="rounded-full border border-amber-300/20 bg-amber-300/10 px-3 py-1 text-xs text-amber-100">📍 {p}</span>
            ))}
          </div>
        )}
      </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {moodInfo && (
              <span className={`text-xs font-medium ${moodInfo.color}`}>
                {journal.mood_score}/10 {moodInfo.label}
              </span>
            )}
          </div>
          <div className="flex shrink-0 gap-2">
